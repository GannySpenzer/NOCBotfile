...
namespace SAErrorReload
{
    class SAErrorReload
    {
        static void Main(string[] args)
        {
            ...
            m_oLogger.LogMessage("Main", "Started utility SAErrorReload");
            ...
            using (var client = new WebClient())
            {
...

                {
                    ...
                    m_oLogger.LogMessage("postSAErrorReload", "QUERYING Oracle Data at " + serviceURL);
                    ...
                    if (json.Trim() != "")
                    {
                        m_oLogger.LogMessage("SAErrorReload", "QUERYING Oracle Data successful");
                    }
                    else
                    {
                        m_oLogger.LogMessage("SAErrorReload", "QUERYING Oracle Data failed");
                        m_oLogger.LogMessageWeb("SAErrorReload", "QUERYING Oracle Data failed", "QUERYING Oracle Data failed");
                        return;
                    }
                    ...
                    m_oLogger.LogMessage("SAErrorReload", "DELETE Oracle Data started");
                    //try
                    //{
                    ...
                    try
                    {
                        ...
                        if (delTimes > 0)
                        {
                            ...
                            {
                                strDelQuery += "Delete From CO.SAError LIMIT 1000;";
                            }
                            client.OpenRead(serviceURL2 + strDelQuery);
                            m_oLogger.LogMessage("SAErrorReload", "DELETE Oracle Data of " + bo.items[0].rows.Count.ToString() + " records successful.");
                        }
                    }
                    catch (Exception ex)
                    {
                        m_oLogger.LogMessage("SAErrorReload", "DELETE Oracle Data failed.");
                        m_oLogger.LogMessageWeb("SAErrorReload", "DELETE Oracle Data failed", "DELETE Oracle Data failed.  Resultset to delete: " + resultSet);
                        return;
                    }

                    try
                    {
                        SAErrorReloadDAL dal = new SAErrorReloadDAL();
                        ...
                        if (dal.dtResponseRowsCount > 0)
                        {
                            ...
                            {
                                SAEData sae = dal.getData(m_oLogger);

                                //new batch SoapUI code
                                Batcher batcher = new Batcher(authorization, password);
                                batcher.CreateBuyExpBatch(sae, m_oLogger, out strResponse);
                ...
                m_oLogger.LogMessage("SAErrorReload", "POST SAErrorReload data to Oracle Helix server status " + strResponse);

                if (strResponse.ToUpper() != "SUCCESS")
                {
                    m_oLogger.LogMessage("SAErrorReload", "POST SAErrorReload data to Oracle Helix server status " + strResponse);
                    m_oLogger.LogMessageWeb("SAErrorReload", "POST SAErrorReload data to Oracle Helix server status " + strResponse, "POST SAErrorReload data to Oracle server status " + strResponse);
            ...
            m_oLogger.LogMessage("Main", "SAErrorReload End");
