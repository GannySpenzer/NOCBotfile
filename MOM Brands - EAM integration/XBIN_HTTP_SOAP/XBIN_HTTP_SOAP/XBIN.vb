Module XBIN
    ...
    Sub Main()
        ...
        Const processCode As String = "XBIN"
        ...
        Try
            s1 = My.Settings(propertyName:="XBIN_GetBatchSize")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        End If
        Try
            s1 = My.Settings(propertyName:="XBIN_MaxReq")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        Dim sql As String = ""
        Dim oBatch As XBIN_batch = Nothing
        ...
        If (oraCN1.State = ConnectionState.Open) Then
            ...
            oBatch = New XBIN_batch

            sr = New System.IO.StreamReader(CStr(My.Settings("XBIN_SelectToSend")))
            s = sr.ReadToEnd
            sr.Dispose()
            sr = Nothing
            ...
            Catch ex As Exception
                oBatch.ex = ex
                logger.WriteErrorLog("errorred retrieving bin-to-bin inventory transfer records to process from table.")
                Try
                    dr.Close()
            ...
            If Not (dr Is Nothing) Then
                If dr.HasRows Then
                    ...
                    Dim sUpdateKeyColumns As String = CStr(My.Settings("XBIN_UpdateKeyColumns"))
                    ...
                    While dr.Read
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("XBIN_HTTP_SOAP_Batch_Item")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        '	1040 	TO BIN
                        '	1050 	QTY
                        '	1060 	ASSET ID (if asset part)        passing <blank> at the moment
                        sb = New System.Text.StringBuilder
                        sb.AppendFormat(s, sPlant, sItem, sFromBin, sToBin, nQty.ToString, sAssetId)
                        iList &= sb.ToString
                        sb = Nothing
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("XBIN_HTTP_SOAP_Batch")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        '   Username {0}
                        '   Password {1}
                        '   "bin-to-bin transfers" {2}
                        sb = New System.Text.StringBuilder
                        sb.AppendFormat(s, oParent.Username, oParent.Password, iList)
                        iBatch = sb.ToString
                        sb = Nothing

                        'logger.WriteVerboseLog("loaded XBIN HTTP SOAP request XML : " & iBatch)
                        logger.WriteVerboseLog("loaded XBIN HTTP SOAP request XML : " & CStr(My.Settings("XBIN_HTTP_SOAP_Batch")))
                        ...
                        Else
                            ...
                            Try
                                bClearErrMsg = CBool(My.Settings("XBIN_ClearErrMsgOnSuccess"))
                            Catch ex As Exception
                            End Try
                        ...
                        '// update our table - PS_ISA_MB_BIN_XFER
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("XBIN_UpdateInputTable")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        Else
                            ...
                            Catch ex As Exception
                                'oBatch.ex = ex
                                logger.WriteErrorLog("errorred updating bin-to-bin transfer record that was processed.")
                            End Try
                ...
                Else        ' If dr.HasRows Then
                    logger.WriteInformationLog("no bin-to-bin transfer record to process.")
                End If
    ...
    Private Function GetDescriptiveErrorMessage(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        If (sRawMsg.Length > 0) Then
            ...
            Try
                sErrCodePattern = CStr(My.Settings("XBIN_CanParseErrMsgSignature")).Trim
            Catch ex As Exception
            End Try
            ...
            Try
                sErrIdPattern = CStr(My.Settings("XBIN_ErrMsgId")).Trim
            Catch ex As Exception
            End Try
            ...
            If (sErrId.Length > 0) Then
                ...
                sr = New System.IO.StreamReader(CStr(My.Settings("XBIN_GetErrMsgDesc")))
                sql = sr.ReadToEnd
                sr.Dispose()
                sr = Nothing
