Module POWO
    ...
    Sub Main()
        ...
        Const processCode As String = "POWO"
        ...
        Dim ackPOs As New POWO_AckBatch
        ...
        ackPOs.ColumnIdToSave = CStr(My.Settings("POWO_AckColumnId")).Trim
        ackPOs.AcknowledgementBatchSize = CInt(My.Settings("POWO_AckBatchSize"))

        '   (2) grab child configuration
        Try
            s1 = My.Settings(propertyName:="POWO_GetBatchSize")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        End If
        Try
            s1 = My.Settings(propertyName:="POWO_MaxReq")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
    ...
    Private Sub RetrieveAndProcess(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        Dim sr As New System.IO.StreamReader(CStr(My.Settings("POWO_HTTP_SOAP_REQ")))
        s = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        ...
        'logger.WriteVerboseLog("loaded PO/WO HTTP SOAP request XML : " & s)
        logger.WriteVerboseLog("loaded PO/WO HTTP SOAP request XML : " & CStr(My.Settings("POWO_HTTP_SOAP_REQ")))
        ...
        xmlTableMap.Load(filename:=CStr(My.Settings("POWO_HTTP_SOAP_DataMap")))
        ...
        logger.WriteVerboseLog("loaded data map : " & CStr(My.Settings("POWO_HTTP_SOAP_DataMap")))
        ...
        If Not (ndData Is Nothing) Then
            For Each ndRow As Xml.XmlNode In ndData.ChildNodes
                If (ndRow.Name.Trim.ToUpper = "ROW") Then
                    ...
                    Dim sAckKeyColumns As String = CStr(My.Settings("POWO_AckKeyColumns"))
    ...
    Private Sub SendAcknowledgement(ByVal processId As String, _
                                    ByVal oParent As SDI.HTTP_SOAP_INTFC.IParentApp, _
                                    ByVal ackPOs As POWO_AckBatch, _
        ...
        ' per batch
        sr = New System.IO.StreamReader(CStr(My.Settings("POWO_HTTP_SOAP_ACK")))
        sAckBatch = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        logger.WriteVerboseLog(CStr(My.Settings("POWO_HTTP_SOAP_ACK")))

        ' per item
        sr = New System.IO.StreamReader(CStr(My.Settings("POWO_HTTP_SOAP_ACK_ITM")))
        sAckItem = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        logger.WriteVerboseLog(CStr(My.Settings("POWO_HTTP_SOAP_ACK_ITM")))
        ...
        For Each sWO As String In ackPOs.UniqueIds.Keys
            ...
            If (nBatchCtr >= ackPOs.AcknowledgementBatchSize) Or (nTotalCtr >= ackPOs.UniqueIds.Keys.Count) Then
                ...
                Catch ex As Exception
                    ...
                    arrPOAcknowledged(CStr(arrPOAcknowledged.Keys(sWO))) = New String() {bIsAckSucceed.ToString, sAckErrMsg}
                End Try
                ...
                For Each sPOAck As String In arrPOAcknowledged.Keys
                    ...
                    If Not bIsAckSucceed Then
                        ...
                        Dim sql As String = "DELETE FROM SYSADM.PS_ISA_MB_REQ_NOPO WHERE PROCESS_INSTANCE = 0 AND {0}"
                        s = ""
                        ...
                        ' log
                        logger.WriteErrorLog("P/O / W/O entry " & sPOAck & " failed acknowledgement with error message :: " & sAckErrMsg)
                        logger.WriteErrorLog("Backed out P/O / W/O entry " & sPOAck & " from table.")
                    End If
                Next
