Module MATMST
    ...
    Sub Main()
        ...
        Const processCode As String = "MASMST"
        ...
        Dim ackMATMSTs As New MATMST_AckBatch
        ...
        ackMATMSTs.ColumnIdToSave = CStr(My.Settings("MATMST_AckColumnId")).Trim
        ackMATMSTs.AcknowledgementBatchSize = CInt(My.Settings("MATMST_AckBatchSize"))

        '   (2) grab child configuration
        Try
            s1 = My.Settings(propertyName:="MATMST_GetBatchSize")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        End If
        Try
            s1 = My.Settings(propertyName:="MATMST_MaxReq")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        Dim sql As String = ""
        Dim oBatch As New MATMST_batch
        ...
        If (oraCN1.State = ConnectionState.Open) Then
            ...
            'sql = "" & _
            '      "DELETE FROM SYSADM.PS_ISA_MB_MATMAST " & _
            '      "WHERE CUST_ID = '{0}' " & _
            '      ""
            ...
            Try
                logger.WriteVerboseLog("sending acknowledgement for " & ackMATMSTs.UniqueIds.Keys.Count.ToString & " inventory master records received.")
                SendAcknowledgement(processIdAck, oParent, ackMATMSTs, oraCN1, custId, oraTran1)
            Catch ex As Exception
                oBatch.ex = ex
    ...
    Private Sub RetrieveAndProcess(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        Dim sr As New System.IO.StreamReader(CStr(My.Settings("MATMST_HTTP_SOAP_REQ")))
        ...
        'logger.WriteVerboseLog("loaded MATMST HTTP SOAP request XML : " & s)
        logger.WriteVerboseLog("loaded MATMST HTTP SOAP request XML : " & CStr(My.Settings("MATMST_HTTP_SOAP_REQ")))
        ...
        xmlTableMap.Load(filename:=CStr(My.Settings("MATMST_HTTP_SOAP_DataMap")))
        ...
        logger.WriteVerboseLog("loaded data map : " & CStr(My.Settings("MATMST_HTTP_SOAP_DataMap")))
    ...
    Private Sub SendAcknowledgement(ByVal processId As String, _
                                    ByVal oParent As SDI.HTTP_SOAP_INTFC.IParentApp, _
                                    ByVal ackMATMSTs As MATMST_AckBatch, _
        ...
        Dim Response_Doc As String = Nothing
        Dim arrMATMSTAcknowledged As Hashtable = Nothing
        Dim bIsAckSucceed As Boolean = False
        Dim sAckErrMsg As String = ""

        ' per batch
        sr = New System.IO.StreamReader(CStr(My.Settings("MATMST_HTTP_SOAP_ACK")))
        sAckBatch = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        logger.WriteVerboseLog(CStr(My.Settings("MATMST_HTTP_SOAP_ACK")))

        ' per item
        sr = New System.IO.StreamReader(CStr(My.Settings("MATMST_HTTP_SOAP_ACK_ITM")))
        sAckItem = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        logger.WriteVerboseLog(CStr(My.Settings("MATMST_HTTP_SOAP_ACK_ITM")))

        sItemList = ""

        For Each sMATMST As String In ackMATMSTs.UniqueIds.Keys

            ' increment counters
            nBatchCtr += 1
            nTotalCtr += 1

            If (arrMATMSTAcknowledged Is Nothing) Then
                arrMATMSTAcknowledged = New Hashtable
            End If
            arrMATMSTAcknowledged.Add(key:=sMATMST, value:=New String() {"false", ""})

            ' build xml string for each item acknowledgement
            sbItem = New System.Text.StringBuilder
            sbItem.AppendFormat(sAckItem, sMATMST, Now.ToString("MM-dd-yyyy HH:mm:ss"))
            sItemList &= sbItem.ToString
            sbItem = Nothing

            ' check if time to send this batch
            If (nBatchCtr >= ackMATMSTs.AcknowledgementBatchSize) Or (nTotalCtr >= ackMATMSTs.UniqueIds.Keys.Count) Then
                Dim idx As String = (nTotalCtr - nBatchCtr + 1).ToString.PadLeft(ackMATMSTs.UniqueIds.Keys.Count.ToString.Length, "0"c)
                Dim flePath As String = oParent.TemporaryFilePath.Trim
                While (flePath.Length > 0) And (flePath.LastIndexOf("\"c) = (flePath.Length - 1))
                    flePath = flePath.TrimEnd("\"c)
                End While
                ...
                '       and update arrMATMSTAcknowledged of each W/O's acknowledgement state
                Dim xmlIn As New System.Xml.XmlDocument
                ...
                For i As Integer = 0 To (nd.ChildNodes.Count - 1) ' actionResult
                    ...
                    arrMATMSTAcknowledged(CStr(arrMATMSTAcknowledged.Keys(i))) = New String() {bIsAckSucceed.ToString, sAckErrMsg}
                Next
                ...
                ' delete all Material Master that acknowledgement fails
                '       and log Mat/Master record with the "failure message"
                For Each sMATMSTAck As String In arrMATMSTAcknowledged.Keys
                    bIsAckSucceed = CBool(CType(arrMATMSTAcknowledged(sMATMSTAck), String())(0))
                    sAckErrMsg = CStr(CType(arrMATMSTAcknowledged(sMATMSTAck), String())(1))
                    If Not bIsAckSucceed Then
                        Dim keyValue As ArrayList = CType(ackMATMSTs.UniqueIds(sMATMSTAck), ArrayList)

                        ' delete w/o entry(ies)
                        Dim sql As String = "DELETE FROM SYSADM.PS_ISA_MB_MATMAST WHERE {0}"
                        s = ""
                        ...
                        ' log
                        logger.WriteErrorLog("Material Master item#" & sMATMSTAck & " failed acknowledgement with error message :: " & sAckErrMsg)
                        logger.WriteErrorLog("Backed out Material Master item# " & sMATMSTAck & " from table.")
                    End If
                Next
