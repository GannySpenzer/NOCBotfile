Module WO
    ...
    Sub Main()
        ...
        Const processCode As String = "WO"
        ...
        ackWOs.ColumnIdToSave = CStr(My.Settings("WO_AckColumnId")).Trim
        ackWOs.AcknowledgementBatchSize = CInt(My.Settings("WO_AckBatchSize"))

        '   (2) grab child configuration
        Try
            s1 = My.Settings(propertyName:="WO_GetBatchSize")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        End If
        Try
            s1 = My.Settings(propertyName:="WO_MaxReq")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        logger.WriteVerboseLog("processing (acknowledgement) batch size " & ackWOs.AcknowledgementBatchSize.ToString & " for a maximum of " & nMaxReq.ToString & " request(s)")
        ...
        If (oraCN1.State = ConnectionState.Open) Then
            ...
            While (nLoopCtr < nMaxReq)
                ...
                Try
                    ...
                    RetrieveAndProcess(oraCN1, custId, processId, oraTran1, oBatch, appVar, oParent, ackWOs)
            ...
            Try
                logger.WriteVerboseLog("sending acknowledgement for " & ackWOs.UniqueIds.Keys.Count.ToString & " W/O received.")
                SendAcknowledgement(processIdAck, oParent, ackWOs, oraCN1, custId, oraTran1)
            Catch ex As Exception
                oBatch.ex = ex
    ...
    Private Sub RetrieveAndProcess(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        'Dim sr As New System.IO.StreamReader("C:\projects\Apps\MOM Brands - EAM integration\WO_HTTP_SOAP\WO_HTTP_SOAP\MOMBrand_WO_HTTP_SOAP_Request.xml")
        Dim sr As New System.IO.StreamReader(CStr(My.Settings("WO_HTTP_SOAP_REQ")))
        s = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        ...
        logger.WriteVerboseLog("loaded WO HTTP SOAP request XML : " & CStr(My.Settings("WO_HTTP_SOAP_REQ")))
        ...
        'xmlTableMap.Load(filename:="C:\projects\Apps\MOM Brands - EAM integration\WO_HTTP_SOAP\WO_HTTP_SOAP\MOMBrand_WO_HTTP_SOAP_TableColMap.xml")
        xmlTableMap.Load(filename:=CStr(My.Settings("WO_HTTP_SOAP_DataMap")))
        ...
        logger.WriteVerboseLog("loaded data map : " & CStr(My.Settings("WO_HTTP_SOAP_DataMap")))
        ...
        '   (2) data (row)
        Dim sColumnNameForAck As String = ackWOs.ColumnIdToSave
        ...
        If Not (ndData Is Nothing) Then
            For Each ndRow As Xml.XmlNode In ndData.ChildNodes
                If (ndRow.Name.Trim.ToUpper = "ROW") Then
                        ...
                        If (idxCol >= 0) Then
                            ...
                            If (CType(dtMap.Columns(idxCol), SDI.HTTP_SOAP_INTFC.CColumn).ColumnName = sColumnNameForAck) Then
                                Try
                                    ...
                                    'End If
                                    If Not ackWOs.UniqueIds.ContainsKey(ndFld.InnerText.Trim.ToUpper) Then
                                        ackWOs.UniqueIds.Add(key:=ndFld.InnerText.Trim.ToUpper, value:=Nothing)
                                    End If
                                Catch ex As Exception
                                    Throw New ApplicationException(ex.ToString)
                                End Try
                    ...
                    Dim sAckKeyColumns As String = CStr(My.Settings("WO_AckKeyColumns"))
    ...
    Private Sub SendAcknowledgement(ByVal processId As String, _
        ...
        Dim Response_Doc As String = Nothing
        Dim arrWOAcknowledged As Hashtable = Nothing
        Dim bIsAckSucceed As Boolean = False
        Dim sAckErrMsg As String = ""

        ' per batch
        sr = New System.IO.StreamReader(CStr(My.Settings("WO_HTTP_SOAP_ACK")))
        sAckBatch = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        logger.WriteVerboseLog(CStr(My.Settings("WO_HTTP_SOAP_ACK")))

        ' per item
        sr = New System.IO.StreamReader(CStr(My.Settings("WO_HTTP_SOAP_ACK_ITM")))
        sAckItem = sr.ReadToEnd
        sr.Dispose()
        sr = Nothing
        logger.WriteVerboseLog(CStr(My.Settings("WO_HTTP_SOAP_ACK_ITM")))

        sItemList = ""

        For Each sWO As String In ackWOs.UniqueIds.Keys

            ' increment counters
            nBatchCtr += 1
            nTotalCtr += 1

            If (arrWOAcknowledged Is Nothing) Then
                arrWOAcknowledged = New Hashtable
            End If
            arrWOAcknowledged.Add(key:=sWO, value:=New String() {"false", ""})
            ...
            If (nBatchCtr >= ackWOs.AcknowledgementBatchSize) Or (nTotalCtr >= ackWOs.UniqueIds.Keys.Count) Then
                Dim idx As String = (nTotalCtr - nBatchCtr + 1).ToString.PadLeft(ackWOs.UniqueIds.Keys.Count.ToString.Length, "0"c)
                'Dim fle As String = "c:\tmp\" & processId & " (" & idx & ").xml"
                Dim flePath As String = oParent.TemporaryFilePath.Trim
                While (flePath.Length > 0) And (flePath.LastIndexOf("\"c) = (flePath.Length - 1))
                    flePath = flePath.TrimEnd("\"c)
                ...
                For Each sWOAck As String In arrWOAcknowledged.Keys
                    bIsAckSucceed = CBool(CType(arrWOAcknowledged(sWOAck), String())(0))
                    sAckErrMsg = CStr(CType(arrWOAcknowledged(sWOAck), String())(1))
                    If Not bIsAckSucceed Then
                        Dim keyValue As ArrayList = CType(ackWOs.UniqueIds(sWOAck), ArrayList)

                        ' delete w/o entry(ies)
                        Dim sql As String = "DELETE FROM SYSADM.PS_ISA_MB_WO_WS WHERE PROCESS_INSTANCE = 0 AND {0}"
                        s = ""
                        ...
                        ' log
                        logger.WriteErrorLog("Work order# " & sWOAck & " failed acknowledgement with error message :: " & sAckErrMsg)
                        logger.WriteErrorLog("Backed out work order# " & sWOAck & " from table.")
                    End If
                Next
