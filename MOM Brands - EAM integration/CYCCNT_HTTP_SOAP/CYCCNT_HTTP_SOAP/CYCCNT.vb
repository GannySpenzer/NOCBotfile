Module CYCCNT
            ...
            If Not (dr Is Nothing) Then
                If dr.HasRows Then
                    ...
                    While dr.Read
                        ...
                        sKey = sEventId & "." & sPlant & "-" & sItem & "." & nRecId.ToString.PadLeft(4, "0"c)
                        ...
                        Else
                            ...
                            Try
                                bClearErrMsg = CBool(My.Settings("CYCCNT_ClearErrMsgOnSuccess"))
                            Catch ex As Exception
                            End Try
    ...
    Private Function GetDescriptiveErrorMessage(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        If (sRawMsg.Length > 0) Then
            ...
            Try
                sErrCodePattern = CStr(My.Settings("CYCCNT_CanParseErrMsgSignature")).Trim
            Catch ex As Exception
            End Try
            ...
            Try
                sErrIdPattern = CStr(My.Settings("CYCCNT_ErrMsgId")).Trim
            Catch ex As Exception
            End Try
            ...
            If (sErrId.Length > 0) Then
                ...
                sr = New System.IO.StreamReader(CStr(My.Settings("CYCCNT_GetErrMsgDesc")))
                sql = sr.ReadToEnd
                sr.Dispose()
                sr = Nothing
