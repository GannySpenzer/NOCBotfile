Module ISSRET
    ...
    Sub Main()
        ...
        Const processCode As String = "ISSRET"
        ...
        Try
            s1 = My.Settings(propertyName:="ISSRET_GetBatchSize")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        End If
        Try
            s1 = My.Settings(propertyName:="ISSRET_MaxReq")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        Dim sql As String = ""
        Dim oBatch As ISSRET_batch = Nothing
        Dim oraCN1 As New OleDb.OleDbConnection(oraCNString)
        Dim sr As System.IO.StreamReader = Nothing
        Dim s As String = ""
        Dim nRec As Integer = 0

        ' transaction code : cross reference
        Dim arrTransCode As New Hashtable
        Dim sTc As String() = New String() {}
        Try
            sTc = CStr(My.Settings("ISSRET_TransactionCodeXRef")).Split(","c)
        Catch ex As Exception
        End Try
        If (sTc Is Nothing) Then
            sTc = New String() {}
        End If
        If (sTc.Length > 0) Then
            For i As Integer = 0 To (sTc.Length - 1)
                Dim sKey2 As String = ""
                Try
                    sKey2 = sTc(i).Split("="c)(0).Trim.ToUpper
                Catch ex As Exception
                End Try
                Dim sVal2 As String = ""
                Try
                    sVal2 = sTc(i).Split("="c)(1).Trim
                Catch ex As Exception
                End Try
                If (sKey2.Length > 0 And sVal2.Length > 0) Then
                    If Not arrTransCode.ContainsKey(sKey2) Then
                        arrTransCode.Add(sKey2, sVal2)
                    End If
                End If
            Next
        End If
        ...
        If (oraCN1.State = ConnectionState.Open) Then
            ...
            oBatch = New ISSRET_batch

            sr = New System.IO.StreamReader(CStr(My.Settings("ISSRET_SelectToSend")))
            s = sr.ReadToEnd
            sr.Dispose()
            sr = Nothing
            ...
            Catch ex As Exception
                oBatch.ex = ex
                logger.WriteErrorLog("errorred retrieving issues/returns to process from table.")
                Try
                    dr.Close()
            ...
            If Not (dr Is Nothing) Then
                If dr.HasRows Then
                    ...
                    Dim sEAM_TransCode As String = ""
                    Dim sWorkOrderNo As String = ""
                    Dim sActivityId As String = ""
                    Dim sEquipmentId As String = ""
                    ...
                    Dim sUpdateKeyColumns As String = CStr(My.Settings("ISSRET_UpdateKeyColumns"))
                    ...
                    While dr.Read
                        ...
                        sTransCode = ""
                        Try
                            sTransCode = CStr(dr("TRANS_CODE")).Trim.ToUpper
                        Catch ex As Exception
                        End Try
                        If (sTransCode Is Nothing) Then
                            sTransCode = ""
                        End If

                        sEAM_TransCode = ""
                        Try
                            sEAM_TransCode = CStr(arrTransCode(sTransCode))
                        Catch ex As Exception
                        End Try
                        If (sEAM_TransCode Is Nothing) Then
                            sEAM_TransCode = ""
                        End If

                        ' if I don't find the "translation" for the transaction code above, I'll send whatever is in the table
                        '   this is to prevent the issue that happened 8/25/2014 that a new transaction code was put in but this process was 
                        '   sending empty transaction code because I wasn't informed about it (ie., RPR for repair) - Erwin
                        If (sEAM_TransCode.Trim.Length = 0) Then
                            sEAM_TransCode = sTransCode
                        End If
                        ...
                        sActivityId = ""
                        Try
                            sActivityId = CStr(dr("ACTIVITY_ID")).Trim
                        Catch ex As Exception
                        End Try
                        If (sActivityId Is Nothing) Then
                            sActivityId = ""
                        End If

                        sEquipmentId = ""
                        Try
                            sEquipmentId = CStr(dr("SERIAL_ID")).Trim
                        Catch ex As Exception
                        End Try
                        If (sEquipmentId Is Nothing) Then
                            sEquipmentId = ""
                        End If
                        ...
                        sInitialProcessFlag = ""
                        Try
                            sInitialProcessFlag = CStr(dr("PROCESS_FLAG")).Trim.ToUpper
                        Catch ex As Exception
                        End Try
                        ...
                        sKey = sTransCode & "." & sPlant & "." & sItem & "." & sWorkOrderNo & "." & nRecId.ToString.PadLeft(4, "0"c)
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("ISSRET_HTTP_SOAP_Batch_Item")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing

                        ' fields
                        '	1010 	Issue or Return
                        '	1020 	Work Order #
                        '	1030 	Activity #
                        '	1040 	Equipment #             passing <blank> at the moment
                        '	1050 	Store
                        '	1060 	Part
                        '	1070 	Bin
                        '	1080 	Qty
                        '	1090 	Asset ID                passing <blank> at the moment
                        sb = New System.Text.StringBuilder
                        sb.AppendFormat(s, sEAM_TransCode, sWorkOrderNo, sActivityId, "", sPlant, sItem, sBin, nQty.ToString, sAssetId, sEmployeeId)
                        iList &= sb.ToString
                        sb = Nothing
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("ISSRET_HTTP_SOAP_Batch")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        logger.WriteVerboseLog("loaded ISSRET HTTP SOAP request XML : " & CStr(My.Settings("ISSRET_HTTP_SOAP_Batch")))
                        ...
                        Else
                            ...
                            Try
                                bClearErrMsg = CBool(My.Settings("ISSRET_ClearErrMsgOnSuccess"))
                            Catch ex As Exception
                            End Try
                        ...
                        '// update our table - PS_ISA_MB_ISS_RET
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("ISSRET_UpdateInputTable")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        Else
                            ...
                            Catch ex As Exception
                                'oBatch.ex = ex
                                logger.WriteErrorLog("errorred updating issue/return record that was processed.")
                            End Try
                ...
                Else        ' If dr.HasRows Then
                    logger.WriteInformationLog("no issue/return to process.")
                End If
    ...
    Private Function GetDescriptiveErrorMessage(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        If (sRawMsg.Length > 0) Then
            ...
            Try
                sErrIdPattern = CStr(My.Settings("ISSRET_ErrMsgId")).Trim
            Catch ex As Exception
            End Try
            ...
            If (sErrId.Length > 0) Then
                ...
                sr = New System.IO.StreamReader(CStr(My.Settings("ISSRET_GetErrMsgDesc")))
                sql = sr.ReadToEnd
                sr.Dispose()
                sr = Nothing
