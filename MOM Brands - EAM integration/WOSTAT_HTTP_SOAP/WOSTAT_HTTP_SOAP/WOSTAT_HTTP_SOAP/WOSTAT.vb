Module WOSTAT
    ...
    Sub Main()
        ...
        Const processCode As String = "WOSTAT"
        ...
        Try
            s1 = My.Settings(propertyName:="WOSTAT_GetBatchSize")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        End If
        Try
            s1 = My.Settings(propertyName:="WOSTAT_MaxReq")
        Catch ex As Exception
            s1 = "0"
        End Try
        If CInt(s1) > 0 Then
        ...
        Dim sql As String = ""
        Dim oBatch As WOSTAT_batch = Nothing
        ...
        If (oraCN1.State = ConnectionState.Open) Then
            ...
            oBatch = New WOSTAT_batch

            sr = New System.IO.StreamReader(CStr(My.Settings("WOSTAT_SelectToSend")))
            s = sr.ReadToEnd
            sr.Dispose()
            sr = Nothing
            ...
            Catch ex As Exception
                oBatch.ex = ex
                logger.WriteErrorLog("errorred retrieving W/O statuses to process from table.")
                Try
                    dr.Close()
            ...
            If Not (dr Is Nothing) Then
                If dr.HasRows Then
                    ...
                    Dim sUpdateKeyColumns As String = CStr(My.Settings("WOSTAT_UpdateKeyColumns"))
                    ...
                    While dr.Read
                        ...
                        sCustId = ""
                        Try
                            sCustId = CStr(dr("CUST_ID")).Trim.ToUpper
                        Catch ex As Exception
                        End Try
                        If (sCustId Is Nothing) Then
                            sCustId = ""
                        End If
                        ...
                        sWorkOrderNo = ""
                        Try
                            sWorkOrderNo = CStr(dr("ISA_WORK_ORDER_NO")).Trim.ToUpper
                        Catch ex As Exception
                        End Try
                        ...
                        sStatus = ""
                        Try
                            sStatus = CStr(dr("ISA_WO_STATUS")).Trim.ToUpper
                        Catch ex As Exception
                        End Try
                        ...
                        sKey = sPlant & "." & sWorkOrderNo & "." & sStatus & "." & nRecId.ToString.PadLeft(4, "0"c)
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("WOSTAT_HTTP_SOAP_Batch_Item")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        sb = New System.Text.StringBuilder
                        sb.AppendFormat(s, _
                                        Now.ToString("MM-dd-yyyy HH:mm:ss"), _
                                        sWorkOrderNo, _
                                        sPlant, _
                                        sStatus)
                        iList &= sb.ToString
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("WOSTAT_HTTP_SOAP_Batch")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        'logger.WriteVerboseLog("loaded WOSTAT HTTP SOAP request XML : " & s)
                        logger.WriteVerboseLog("loaded WOSTAT HTTP SOAP request XML : " & CStr(My.Settings("WOSTAT_HTTP_SOAP_Batch")))
                        ...
                        nd = xmlIn.SelectSingleNode("//cr3:result", nsmgr)

                        If Not (nd Is Nothing) Then
                            For i As Integer = 0 To (nd.ChildNodes.Count - 1) ' actionResult
                                bSucceed = False
                                nd1 = nd.ChildNodes(i).SelectSingleNode("cr3:success", nsmgr)
                                If Not (nd1 Is Nothing) Then
                                    s = nd1.InnerText.Trim.ToUpper
                                    bSucceed = (s = "TRUE")
                                End If
                                nd1 = Nothing
                        ...
                        Else
                            ...
                            Try
                                bClearErrMsg = CBool(My.Settings("WOSTAT_ClearErrMsgOnSuccess"))
                            Catch ex As Exception
                            End Try
                        ...
                        '// update our table - PS_ISA_MB_STAT_OUT
                        ...
                        sr = New System.IO.StreamReader(CStr(My.Settings("WOSTAT_UpdateInputTable")))
                        s = sr.ReadToEnd
                        sr.Dispose()
                        sr = Nothing
                        ...
                        Else
                            ...
                            Catch ex As Exception
                                'oBatch.ex = ex
                                logger.WriteErrorLog("errorred updating W/O status change record that was processed.")
                            End Try
                ...
                Else        ' If dr.HasRows Then
                    logger.WriteInformationLog("no W/O status change to process.")
                End If
    ...
    Private Function GetDescriptiveErrorMessage(ByVal oraCN As OleDb.OleDbConnection, _
        ...
        If (sRawMsg.Length > 0) Then
            ...
            Try
                sErrCodePattern = CStr(My.Settings("WOSTAT_CanParseErrMsgSignature")).Trim
            Catch ex As Exception
            End Try
            ...
            Try
                sErrIdPattern = CStr(My.Settings("WOSTAT_ErrMsgId")).Trim
            Catch ex As Exception
            End Try
            ...
            If (sErrId.Length > 0) Then
                ...
                sr = New System.IO.StreamReader(CStr(My.Settings("WOSTAT_GetErrMsgDesc")))
                sql = sr.ReadToEnd
                sr.Dispose()
                sr = Nothing
