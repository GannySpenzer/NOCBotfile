...
'' sample queries
'SELECT DISTINCT A.INVOICE_ID FROM sysadm8.PS_ISA_XEEV_INV_HB A, sysadm8.ps_BI_HDR B where B.BILL_TO_CUST_ID = '90584' 
'AND A.ISA_GST_TAX_AMT = 1 and A.INVOICE_ID=B.INVOICE AND A.BUSINESS_UNIT=B.BUSINESS_UNIT;

'SELECT A.*, B.DUE_DT 
'                    FROM sysadm8.PS_ISA_XEEV_INV_HB A, sysadm8.ps_BI_HDR B 
'                    where B.BILL_TO_CUST_ID = '90584'and A.INVOICE_ID = '11209930' 
'                    AND A.ISA_GST_TAX_AMT = 1 
'                    and A.INVOICE_ID=B.INVOICE AND A.BUSINESS_UNIT=B.BUSINESS_UNIT;

'  SELECT A.* FROM sysadm8.PS_ISA_XEEV_INV_LB A, sysadm8.PS_ISA_XEEV_INV_HB B where A.INVOICE_ID = '11209930' AND B.ISA_GST_TAX_AMT = 1 
'  AND A.INVOICE_ID=B.INVOICE_ID AND A.BUSINESS_UNIT=B.BUSINESS_UNIT AND A.PROCESS_INSTANCE=B.PROCESS_INSTANCE;

'  UPDATE SYSADM8.PS_ISA_XEEV_INV_HB SET ISA_GST_TAX_AMT = 0 WHERE INVOICE_ID = '11209930' and ISA_GST_TAX_AMT = 1;
