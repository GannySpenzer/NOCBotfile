...
Public Class orderStatusCheckerI0260
    ...
    Private m_Special_logger As SDI.ApplicationLogger.IApplicationLogger = Nothing
    Dim strLogFileName As String = "SpecialOrdersNYCOut" & Now.Year & Now.Month & Now.Day & Now.GetHashCode & ".txt"
    Private Logpath1 As String = Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly.GetModules()(0).FullyQualifiedName) & "\LOGS\" & strLogFileName
    ...
    Private m_deviceDeliveredDocLinkURL As String = "https://www.sdiexchange.com/Trucking/shipmentBOL.aspx?bu={0}&ord={1}"
...
#Region " ICanCheckOrderShippedStatus Implementations "
    ...
    Public Sub sendOrderShippedStatusEmail(ByVal sender As Object, ByVal e As orderShippedStatusEventArgs) Implements ICanCheckOrderShippedStatus.sendOrderShippedStatusEmail
        ...
        m_Special_logger = New ApplicationLogger(Logpath1, logLevel)
        Dim rtn As String = "orderStatEmailOut.orderStatusCheckerI0260.sendOrderShippedStatusEmail"
        m_Special_logger.WriteVerboseLog(rtn & ": Start of the Special Orders List ")
        m_Special_logger.WriteVerboseLog(" Order(s) which have at least 1 line with IN_FULLFILL_STATE < 70 ")
        m_Special_logger.WriteVerboseLog("  ")
        Dim iMyCount32 As Integer = 0

        ' get link for scanned document  at least 1 line has IN_FULLFILL_STATE < 70
        Dim s As String = ""
        Try
            s = CStr(m_appSettings(APP_SETTING_SCANNED_DOC_LINK))
        ...
        Try
            s = CStr(m_appSettings(APP_SETTING_DEVICE_DELIV_DOC_LINK))
            If (s Is Nothing) Then
                s = ""
            End If
        Catch ex As Exception
        ...
        Try
            s = CStr(m_appSettings(APP_SETTING_LEGATO_CN))
            If (s Is Nothing) Then
                s = ""
            End If
        Catch ex As Exception
        ...
        If arrOrders.Count > 0 Then
            For Each ord As changedOrder In arrOrders
                ...
                Dim sql As String = "" & _
                                    ...
                                    " ,PS_ISA_USERS_TBL C " & vbCrLf & _
                                    " ,PS_MASTER_ITEM_TBL I" & vbCrLf & _
                                    " ,SYSADM8.PS_IN_DEMAND E" & vbCrLf & _
                                    " ,(" & vbCrLf & _
                                    ...
                                    "     AND ROWNUM < 2 " & vbCrLf & _
                                    "  ) F " & vbCrLf & _
                                    " WHERE A.BUSINESS_UNIT_OM = '" & ord.BusinessUnit & "' " & vbCrLf & _
                                    ...
                                    "   AND E.IN_FULFILL_STATE = '70' " & vbCrLf & _
                                    "   AND E.DEMAND_SOURCE = 'OM' " & vbCrLf & _
                                    "   AND A.BUSINESS_UNIT_OM = E.SOURCE_BUS_UNIT " & vbCrLf & _
                                    "   AND A.ORDER_NO = E.ORDER_NO " & vbCrLf & _
                                    "   AND B.ISA_INTFC_LN = E.ORDER_INT_LINE_NO " & vbCrLf & _
                                    ...
                                    "                     AND G.ISA_LINE_STATUS = 'PKF'" & vbCrLf & _
                                    "                  )" & vbCrLf & _
                                    " UNION" & vbCrLf & _
                                    " SELECT " & vbCrLf & _
                                    "  A.BUSINESS_UNIT_OM AS BUSINESS_UNIT " & vbCrLf & _
                ...
                If arr.Count > 0 Then
                    For Each o As orderHdr In arr
                        If o.OrderLines.Count > 0 Then
                            Dim bIsFullyShipped As Boolean = True
                            Dim bStatusIsNotDLF As Boolean = False
                            If o.OrderNo.Substring(0, 4) = "NY0T" Then
                                ' just log that we didn't generate the email
                                m_logger.WriteVerboseLog("skipped email generation for order " & o.OrderNo & " due that it's starts with  NY0T.")
                            Else
                                'check is order not fully shipped
                                Dim strSql32 As String = ""
                                strSql32 = " SELECT * FROM SYSADM8.PS_IN_DEMAND BB1 " & vbCrLf & _
                                    " WHERE BB1.IN_FULFILL_STATE < '70' " & vbCrLf & _
                                    " AND BB1.SOURCE_BUS_UNIT = 'I0260'  " & vbCrLf & _
                                    " AND BB1.ORDER_NO = '" & o.OrderNo & "' " & vbCrLf & _
                                    "  "
                                ...
                                Dim cmd32 As OleDbCommand = m_oraCN.CreateCommand
                                cmd32.CommandText = strSql32
                                cmd32.CommandType = CommandType.Text

                                Dim UserdataSet As System.Data.DataSet = New System.Data.DataSet()

                                Try
                                    Dim dataAdapter As OleDbDataAdapter = New OleDbDataAdapter(cmd32)
                                    ...
                                    If Not UserdataSet Is Nothing Then
                                        ...
                                        End If
                                    End If
                                    If bIsFullyShipped Then
                                        'check for statuses != 'DLF'
                                        UserdataSet = New System.Data.DataSet()
                                        cmd32 = Nothing

                                        cmd32 = m_oraCN.CreateCommand
                                        strSql32 = "SELECT A.* FROM SYSADM8.PS_ISA_ORD_INTF_LN A WHERE A.ORDER_NO = '" & o.OrderNo & "' " & vbCrLf & _
                                            " AND A.ISA_LINE_STATUS <> 'DLF' " & vbCrLf & _
                                            " AND A.ISA_LINE_STATUS <> 'CNC' " & vbCrLf & _
                                            " AND A.BUSINESS_UNIT_OM = 'I0260' " & vbCrLf & _
                                            " AND (A.ISA_INTFC_LN < 100 " & vbCrLf & _
                                            " OR  A.INV_ITEM_ID = ' ') " & vbCrLf & _
                                            "  "

                                        cmd32.CommandText = strSql32
                                        cmd32.CommandType = CommandType.Text
                                        dataAdapter = New OleDbDataAdapter(cmd32)
                                        ...
                                        If Not UserdataSet Is Nothing Then
                                            If UserdataSet.Tables.Count > 0 Then
                                                If UserdataSet.Tables(0).Rows.Count > 0 Then
                                                    bIsFullyShipped = False
                                                    bStatusIsNotDLF = True
                                                End If
                                            End If
                                        End If
                                    End If  '  If bIsFullyShipped Then
                                    Try
                                        dataAdapter.Dispose()
                                    Catch ex As Exception
                                ...
                                If bIsFullyShipped Then
                                    ...
                                    End If  '  If Not o.IsFabOrder Then
                                End If

                            End If  ' If o.OrderNo.Substring(0, 4) = "NY0T" Then

                            If bIsFullyShipped Then
                                Me.flagOrderAsProcessed(o)
                                m_logger.WriteVerboseLog("flagged order " & o.OrderNo & " as processed.")
                            Else
                                'add this order to the Special list of not Processed Orders
                                If bStatusIsNotDLF Then
                                    m_Special_logger.WriteVerboseLog(" Order: " & o.OrderNo & " is not processed: not all statuses 'DLF' ")
                                Else
                                    m_Special_logger.WriteVerboseLog(" Order: " & o.OrderNo & " is not processed: there are IN_FULFILL_STATE < 70 ")
                                End If
                                
                                iMyCount32 = iMyCount32 + 1

                            End If

                        End If  ' If o.OrderLines.Count > 0 Then
                    Next
                End If
            ...
            m_Special_logger.WriteVerboseLog("  ")
            m_Special_logger.WriteVerboseLog(" End of the Special Orders List ")
            If iMyCount32 > 0 Then
                'send special e-mail 
                SendEmailSpecialOrders(strLogFileName, Logpath1)
            End If
        End If
    ...
    Private Sub SendEmailSpecialOrders(ByVal strAttachedFileName As String, ByVal strFileUrl As String)

        Dim rtn As String = "SendEmailSpecialOrders"
        ...
        'get Special To List - Ron.Fijalkowski@sdi.com; Scott.Doyle@sdi.com
        Dim strList As String = "vitaly.rovensky@sdi.com"
        Try
            strList = My.Settings("SpecialToList").ToString.Trim
        Catch ex As Exception
            strList = "vitaly.rovensky@sdi.com"
        End Try
        ...
        email.Subject = "NYC Special Orders List - not sent due to at least 1 line with IN_FULLFILL_STATE < 70 OR not all lines 'DLF' "

        If m_oraCN.DataSource.ToUpper.IndexOf("RPTG") > -1 Or _
           m_oraCN.DataSource.ToUpper.IndexOf("DEVL") > -1 Or _
           m_oraCN.DataSource.ToUpper.IndexOf("STAR") > -1 Or _
        ...
        email.Body = "<table><tr><td>Please, see attached Summary for the NYC Special Orders List - not sent due to at least 1 line with IN_FULLFILL_STATE < 70 OR not all lines 'DLF' </td></tr></table>"
        ...
        Try
            ...
            SendLoggerAllParams(email.Subject, email.Body, "ORDERSTATEMAILOUT", "Mail", email.To, " ", email.Bcc, MailAttachmentName, MailAttachmentbytes.ToArray())
            bSend = True
        Catch ex As Exception
            m_logger.WriteErrorLog(rtn & " :: Email with special orders list was not sent. Error: " & ex.Message)
            bSend = True
        End Try

        If Not bSend Then
            m_logger.WriteErrorLog(rtn & " :: bSend is False - the email with special orders list was not sent.")
        End If
    End Sub
...
#Region " ICanCheckNSTKOrderShippedStatus Implementations "
    ...
    Public Sub sendNSTKOrderShippedStatusEmail(ByVal sender As Object, ByVal e As orderShippedStatusEventArgs) Implements ICanCheckNSTKOrderShippedStatus.sendNSTKOrderShippedStatusEmail
        ...
        Dim dtStart As DateTime  '   = Me.StartDate
        'get 
        Dim strNSTKStartDate As String = "10/10/2019"
        Try
            strNSTKStartDate = My.Settings("NSTKStartDT").ToString.Trim
            If IsDate(strNSTKStartDate) Then
                'do nothing
            Else
                strNSTKStartDate = "10/10/2019"
            End If
        Catch ex As Exception
            strNSTKStartDate = "10/10/2019"
        End Try
        ...
        If IsDate(strNSTKStartDate) Then
            dtStart = CDate(strNSTKStartDate)
        End If
        Dim dtEnd As DateTime = Now
        ...
        If arrOrders.Count > 0 Then
            For Each ord As changedOrder In arrOrders
                ...
                Dim sql As String = "" & _
                                    ...
                                    " ,L.ISA_MACHINE_NO AS ISA_MACHINE_NO " & vbCrLf & _
                                    " ,L.ISA_WORK_ORDER_NO AS ISA_WORK_ORDER_NO " & vbCrLf & _
                                    " ,(" & vbCrLf & _
                                    "    SELECT LOC.DESCR AS DESCR " & vbCrLf & _
                                    "    FROM PS_LOCATION_TBL LOC" & vbCrLf & _
                                    "    WHERE LOC.SETID='MAIN1'" & vbCrLf & _
                                    ...
                                    "   ,H1.REQ_LINE_NBR" & vbCrLf & _
                                    "  ) H" & vbCrLf & _
                                    " ,PS_ISA_ORD_INTF_HD I, PS_ISA_ORD_INTF_LN L " & vbCrLf & _
                                    ...
                                    "   AND '" & ord.BusinessUnit & "' = I.BUSINESS_UNIT_OM AND I.BUSINESS_UNIT_OM = L.BUSINESS_UNIT_OM " & vbCrLf & _
                                    "   AND A.REQ_ID = I.ORDER_NO AND I.ORDER_NO = L.ORDER_NO AND B.LINE_NBR = L.Isa_intfc_ln " & vbCrLf & _
                                    "   AND '" & ord.BusinessUnit & "' = C.BUSINESS_UNIT (+) " & vbCrLf & _
                                    "   AND L.ISA_EMPLOYEE_ID = C.ISA_EMPLOYEE_ID (+) " & vbCrLf & _
                ...
                If arr.Count > 0 Then
                    For Each o As orderHdr In arr
                        If o.OrderLines.Count > 0 Then
                            If o.OrderNo.Substring(0, 4) = "NY0T" Then
                                ' just log that we didn't generate the email
                                m_logger.WriteVerboseLog("skipped email generation for NON-STOCK order " & o.OrderNo & " due that it's starts with  NY0T.")
                            Else
...
#End Region
    ...
    Private Function getShippedOrders(ByVal bu As String, _
        ...
        Const ordStatusLog_SHIPPED As String = "DLF"  '  "6"
        Dim arr As New ArrayList
        ...
        Dim sql As String = "" & vbCrLf & _
                            ...
                            " FROM PS_ISAORDSTATUSLOG A, SYSADM8.PS_IN_DEMAND B" & vbCrLf & _
                            ...
                            "  AND A.ISA_LINE_STATUS = '" & ordStatusLog_SHIPPED & "' " & vbCrLf & _
                                    "   AND B.IN_FULFILL_STATE = '70' " & vbCrLf & _
                                    "   AND A.BUSINESS_UNIT_OM = B.SOURCE_BUS_UNIT " & vbCrLf & _
                                    "   AND A.ORDER_NO = B.ORDER_NO " & vbCrLf & _
    ...
    Private Sub sendEmailNotification(ByVal order As orderHdr, _
        ...
        msg.From = "SDIExchange@SDI.com"
        msg.Cc = ""

        msg.Bcc = "webdev@sdi.com"
        ...
        If Not (bccEmails Is Nothing) Then
            If bccEmails.Trim.Length > 0 Then
                msg.Bcc = "webdev@sdi.com;" & bccEmails
            End If
        End If
        ...
        Dim msgBody As String = "" & "<img src='https://www.sdiexchange.com/images/SDILogo_Email.png' alt='SDI' width='98px' height='182px' vspace='0' hspace='0' />"

        msgBody &= "<center><span style='font-family:Arial;font-size:X-Large;width:256px;'>SDI Marketplace</span></center>" & vbCrLf
        msgBody &= "<center><span >SDiExchange - Order Status</span></center>"
        msgBody &= "&nbsp;" & vbCrLf
        ...
        msgOrderDetail &= "&nbsp;<BR>"
        msgOrderDetail &= "Your SDiExchange order number " & order.OrderNo & " has been processed and delivered/picked up.<br>"
        msgOrderDetail &= "&nbsp;<BR>"
        ...
        msg.Subject = "SDiExchange - Delivery Notification of Order Number " & order.OrderNo & ""
        msg.Body = msgBody & msgOrderDetail & "<BR><BR><HR width='100%' SIZE='1'><img src='https://www.sdiexchange.com/Images/SDIFooter_Email.png' />"
        ...
        If m_oraCN.DataSource.ToUpper.IndexOf("RPTG") > -1 Or _
           ...
           m_oraCN.DataSource.ToUpper.IndexOf("STAR") > -1 Or _
           m_oraCN.DataSource.ToUpper.IndexOf("PLGR") > -1 Then
            msg.To = "WEBDEV@sdi.com"
            msg.Subject = " (test run) " & msg.Subject
        Else
            msg.To = strPurchaserEmail
        End If
        'msg.To = strPurchaserEmail

        ' msg.Bcc = "Ron.Fijalkowski@sdi.com;" & msg.Bcc

        SendLogger(msg.Subject, msg.Body, "ORDERSTATEMLOUT", "Mail", msg.To, msg.Cc, msg.Bcc)

        'check are there order record for this order number: order.OrderNo - in incident table with status 'Late'
        ' close if exists (update Incidents table)
        Dim strIncidntSql As String = ""
        strIncidntSql = "UPDATE PS_ISA_CP_CCNOTES SET STATUS = 'C', NOTES_1000 = 'Order is delivered. This incident is closed by system ' WHERE ORDER_NO = '" & order.OrderNo & "' " & vbCrLf & _
            " AND ISA_INCDNT_REASON = 'Late' AND STATUS = 'A' AND BUSINESS_UNIT = 'I0260'"
        ...
        Dim i332 As Integer = 0

        cmd = m_oraCN.CreateCommand
        cmd.CommandText = strIncidntSql
        cmd.CommandType = CommandType.Text
        Try
            i332 = cmd.ExecuteNonQuery
            If i332 > 0 Then
                'add log record
                m_logger.WriteVerboseLog("Corresponding to this order: " & order.OrderNo & "  'Late' Incidents are closed by System.")
            End If
        Catch ex As Exception
        End Try
        Try
    ...
    Private Function getShippedNSTKOrders(ByVal bu As String, _
        ...
        ' sample strEmployeeList --> 'EWOJCIK','JVILLAN'
        Try
            strEmployeeList = My.Settings("EmployeeList").ToString.Trim
        ...
        Dim sql As String = " SELECT Z.ORDER_NO, Z.BUSINESS_UNIT_OM, L.ISA_EMPLOYEE_ID FROM ( " & vbCrLf & _
                            " SELECT " & vbCrLf & _
                            " A.ORDER_NO " & vbCrLf & _
                            " ,A.BUSINESS_UNIT_OM " & vbCrLf & _
                            ...
                            " WHERE (A.DTTM_STAMP BETWEEN TO_DATE('" & dtStart.ToString("MM/dd/yyyy HH:mm:ss") & "','MM/DD/YYYY HH24:MI:SS') AND TO_DATE('" & dtEnd.ToString("MM/dd/yyyy HH:mm:ss") & "','MM/DD/YYYY HH24:MI:SS')) " & vbCrLf & _
                            "  AND A.BUSINESS_UNIT_OM = '" & bu & "' " & vbCrLf & _
                            "  AND A.ISA_LINE_STATUS IN ('DLF','RCF') " & vbCrLf & _
                            "  AND NOT EXISTS (" & vbCrLf & _
                            "                  SELECT 'X'" & vbCrLf & _
                            ...
                            "                  WHERE A.BUSINESS_UNIT_OM = G.BUSINESS_UNIT_OM" & vbCrLf & _
                            "                   AND A.ORDER_NO = G.ORDER_NO " & vbCrLf & _
                            "                   AND A.ISA_INTFC_LN = G.LINE_NBR " & vbCrLf & _
                            "                   AND 0 = G.ORDER_INT_LINE_NO " & vbCrLf & _
                            "                   AND 0 = G.DEMAND_LINE_NO " & vbCrLf & _
                            "                   AND G.ISA_LINE_STATUS = 'PKF') " & vbCrLf & _
                            " GROUP BY A.ORDER_NO, A.BUSINESS_UNIT_OM ) Z, SYSADM8.PS_ISA_ORD_INTF_LN L WHERE " & vbCrLf

        ' sample strEmployeeList --> 'EWOJCIK','JVILLAN'
        If Trim(strEmployeeList) <> "" Then
            sql = sql & " L.ISA_EMPLOYEE_ID IN (" & strEmployeeList & ") AND " & vbCrLf
        End If
        sql = sql & " Z.BUSINESS_UNIT_OM = L.BUSINESS_UNIT_OM AND Z.ORDER_NO = L.ORDER_NO " & vbCrLf & _
                            " GROUP BY Z.ORDER_NO, Z.BUSINESS_UNIT_OM, L.ISA_EMPLOYEE_ID " & vbCrLf & _
                            "" & vbCrLf & _
                            ""
