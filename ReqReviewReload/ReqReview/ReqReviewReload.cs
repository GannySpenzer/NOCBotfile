...
namespace ReqReviewReload
{
    class ReqReviewReload
    {
        static void Main(string[] args)
        {
            ...
            m_oLogger.LogMessage("Main", "Started utility ReqReviewReload");
            ...
            using (var client = new WebClient())
            {
...

                {
                    ...
                    m_oLogger.LogMessage("postReqReviewReload", "QUERYING Oracle Data at " + serviceURL);
                    ...
                    if (json.Trim() != "")
                    {
                        m_oLogger.LogMessage("ReqReviewReload", "QUERYING Oracle Data successful");
                    }
                    else
                    {
                        m_oLogger.LogMessage("ReqReviewReload", "QUERYING Oracle Data failed");
                        m_oLogger.LogMessageWeb("ReqReviewReload", "QUERYING Oracle Data failed", "QUERYING Oracle Data failed");
                        return;
                    }
                    ...
                    m_oLogger.LogMessage("ReqReviewReload", "DELETE Oracle Data started");
                    //try
                    //{
                    ...
                    try
                    {
                        ...
                        if (delTimes > 0)
                        {
                            ...
                            {
                                strDelQuery += "Delete From CO.ReqReview LIMIT 1000;";
                            }
                            client.OpenRead(serviceURL2 + strDelQuery);
                            m_oLogger.LogMessage("ReqReviewReload", "DELETE Oracle Data of " + bo.items[0].rows.Count.ToString() + " records successful.");
                        }
                    }
                    catch (Exception ex)
                    {
                        m_oLogger.LogMessage("ReqReviewReload", "DELETE Oracle Data failed.");
                        m_oLogger.LogMessageWeb("ReqReviewReload", "DELETE Oracle Data failed", "DELETE Oracle Data failed.  Resultset to delete: " + resultSet);
                        return;
                    }

                    try
                    {
                        ReqReviewReloadDAL dal = new ReqReviewReloadDAL();
                        ...
                        if (dal.dtResponseRowsCount > 0)
                        {
                            ...
                            {
                                REQData req = dal.getData(m_oLogger);

                                //new batch SoapUI code
                                Batcher batcher = new Batcher(authorization, password);
                                batcher.CreateBuyExpBatch(req, m_oLogger, out strResponse);
                ...
                m_oLogger.LogMessage("ReqReviewReload", "POST ReqReviewReload data to Oracle Helix server status " + strResponse);

                if (strResponse.ToUpper() != "SUCCESS")
                {
                    m_oLogger.LogMessage("ReqReviewReload", "POST ReqReviewReload data to Oracle Helix server status " + strResponse);
                    m_oLogger.LogMessageWeb("ReqReviewReload", "POST ReqReviewReload data to Oracle Helix server status " + strResponse, "POST ReqReviewReload data to Oracle server status " + strResponse);
            ...
            m_oLogger.LogMessage("Main", "ReqReviewReload End");
