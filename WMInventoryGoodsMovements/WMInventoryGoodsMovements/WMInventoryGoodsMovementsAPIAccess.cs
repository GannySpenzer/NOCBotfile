...
#End Region
 ...
 "    AND B.PYMNT_ID      = A.PYMNT_ID(+)" & vbCrLf &
    ...
    Private Sub buildpymntstatus()
        ...
        Dim strSQLstring As String = "SELECT A.FIELDVALUE, A.XLATLONGNAME" & vbCrLf & _
                " FROM SYSADM8.PSXLATITEM A" & vbCrLf & _
                " WHERE A.EFFDT =" & vbCrLf & _
                " (SELECT MAX(A_ED.EFFDT) FROM SYSADM8.PSXLATITEM A_ED" & vbCrLf & _
                " WHERE A.FIELDNAME = A_ED.FIELDNAME" & vbCrLf & _
                " AND A.FIELDVALUE = A_ED.FIELDVALUE" & vbCrLf & _
                " AND A_ED.EFFDT <= SYSDATE)" & vbCrLf & _
                " AND A.FIELDNAME = 'PYMNT_STATUS'" & vbCrLf & _
                " AND A.EFF_STATUS = 'A'"
