...
namespace PODispatchReload
{
    class PODispatchReload
    {
        static void Main(string[] args)
        {
            ...
            m_oLogger.LogMessage("Main", "Started utility PODispatchReload");
            ...
            using (var client = new WebClient())
            {
...

                {
                    ...
                    m_oLogger.LogMessage("postPODispatchReload", "QUERYING Oracle Data at " + serviceURL);
                    ...
                    if (json.Trim() != "")
                    {
                        m_oLogger.LogMessage("PODispatchReload", "QUERYING Oracle Data successful");
                    }
                    else
                    {
                        m_oLogger.LogMessage("PODispatchReload", "QUERYING Oracle Data failed");
                        m_oLogger.LogMessageWeb("PODispatchReload", "QUERYING Oracle Data failed", "QUERYING Oracle Data failed");
                        return;
                    }
                    ...
                    m_oLogger.LogMessage("PODispatchReload", "DELETE Oracle Data started");
                    //try
                    //{
                    ...
                    try
                    {
                        ...
                        if (delTimes > 0)
                        {
                            ...
                            {
                                strDelQuery += "Delete From CO.PODispatch LIMIT 1000;";
                            }
                            client.OpenRead(serviceURL2 + strDelQuery);
                            m_oLogger.LogMessage("PODispatchReload", "DELETE Oracle Data of " + bo.items[0].rows.Count.ToString() + " records successful.");
                        }
                    }
                    catch (Exception ex)
                    {
                        m_oLogger.LogMessage("PODispatchReload", "DELETE Oracle Data failed.");
                        m_oLogger.LogMessageWeb("PODispatchReload", "DELETE Oracle Data failed", "DELETE Oracle Data failed.  Resultset to delete: " + resultSet);
                        return;
                    }

                    try
                    {
                        PODispatchReloadDAL dal = new PODispatchReloadDAL();
                ...
                m_oLogger.LogMessage("PODispatchReload", "POST PODispatchReload data to Oracle Helix server status " + strResponse);

                if (strResponse.ToUpper() != "SUCCESS")
                {
                    m_oLogger.LogMessage("PODispatchReload", "POST PODispatchReload data to Oracle Helix server status " + strResponse);
                    m_oLogger.LogMessageWeb("PODispatchReload", "POST PODispatchReload data to Oracle Helix server status " + strResponse, "POST PODispatchReload data to Oracle server status " + strResponse);
            ...
            m_oLogger.LogMessage("Main", "PODispatchReload End");
