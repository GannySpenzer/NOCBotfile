    ...
    class WMReceiptsMappingDAL
    {
        ...
        public DataTable getWMReceiptMappingData(Logger m_oLogger)
        {
            ...
            try
            {
                ...
                m_oLogger.LogMessage("getWMReceiptMappingData", "Number of rows Seleted " + dtResponse.Rows.Count);
            }
            catch (Exception ex)
            {
                m_oLogger.LogMessage("getWMReceiptMappingData", "Error trying to get the PO Receipt Mapping data.", ex);
        ...
        public int UpdateWMReceiptMappingData(Logger m_oLogger, string ProcFlag)
        {
            ...
            try
            {
                ...
                strSQLstring = "UPDATE SYSADM8.PS_ISA_SV_MOV_OUT SET PROCESS_FLAG='" + ProcFlag + "', DATE_PROCESSED = SYSDATE WHERE PROCESS_FLAG = 'N' AND CUST_ID = 'SOLVAY' AND TRANS_TYPE = '101'";
                ...
                m_oLogger.LogMessage("UpdateWMReceiptMappingData", "Number of rows updated : " + rowsAffected);

            }
            catch (Exception ex)
            {
                m_oLogger.LogMessage("UpdateWMReceiptMappingData", "Error trying to Update the PO Receipt Mapping data.", ex); throw;
            }
            return rowsAffected;
        }
