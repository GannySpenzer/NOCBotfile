...
{
    class WMReceiptsMappingAPIAccess
    {
        /// <summary>
        /// POST Receiving data to the Solvay service
        /// </summary>
        /// <returns></returns>
        public string postWMReceiptMappingData(Logger m_oLogger)
        {
            string testOrProd = " ";
            ...
            try
            {
                ...
                m_oLogger.LogMessage("getWMReceiptMappingData", "Getting WM Receipt Mapping Data starts here");
                dtResponse = objWMReceiptsMappingDAL.getWMReceiptMappingData(m_oLogger);
                ...
                if (dtResponse.Rows.Count != 0)
                {
                    ...
                    for (int i = 0; i <= dtResponse.Rows.Count -1; i++)
                    {
                        ...
                        List<WMReceiptsMappingBO> target = dtResponse2.AsEnumerable()
                        ...
                        using (var client = new WebClient())
                        {
                            ...
                            m_oLogger.LogMessage("postWMReceiptMappingData", "POST WMReceiptMapping Data URL : " + serviceURL);
                                ...
                                catch (Exception ex)
                                {
                                    ...
                                    m_oLogger.LogMessage("postWMReceiptMappingData", "Authenication failed " + ex.Message);
                                }
                            ...
                            m_oLogger.LogMessage("postWMReceiptMappingData", "POST WMReceiptMapping data to Solvay server status " + strResponse);
                                ...
                                if (strResponse.ToUpper() != " ")
                                {
                                    ...
                                    objWMReceiptsMappingDAL.UpdateWMReceiptMappingData(m_oLogger, processFlag, ISA_IDENTIFIER);
                                }
                                else
                                {
                                    m_oLogger.LogMessageWeb("postWMReceiptMappingData", "Error trying to POST data to Solvay server.", responseErrorText); //ex
                                }
                    ...
                    {
                            m_oLogger.LogMessage("postWMReceiptMappingData", "The transaction is not belong to 101");
                    }
                }
            }
