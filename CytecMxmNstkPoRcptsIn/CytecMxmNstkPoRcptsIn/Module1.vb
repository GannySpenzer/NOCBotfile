...
Module Module1
    ...
    Private m_logger As appLogger = Nothing  '  NstkPoRecpts
    Private myLoggr1 As appLogger = Nothing

    Dim rootDir As String = "C:\CytecMxmIn"
    Dim logpath As String = "C:\CytecMxmIn\LOGS\UpdCytecNstkPoRecptsXmlIn" & Now.Year & Now.Month & Now.Day & Now.GetHashCode & ".txt"
    Dim sErrLogPath As String = "C:\CytecMxmIn\LOGS\ErredSQLsNstkPoRecpts" & Now.Year & Now.Month & Now.Day & Now.GetHashCode & ".txt"
    ...
    Sub Main()
        ...
        If (sLogPath.Length > 0) Then
            logpath = sLogPath & "\UpdCytecNstkPoRecptsXmlIn" & Now.Year & Now.Month & Now.Day & Now.GetHashCode & ".txt"
        End If
        ...
        Call ProceesCytecMxmNstkPoRecptsInfo()
    ...
    Private Sub ProceesCytecMxmNstkPoRecptsInfo()

        Dim rtn As String = "CytecMxmNstkPoRecpts.ProceesCytecMxmNstkPoRecptsInfo"
        Dim bError As Boolean = False

        Console.WriteLine("Start CytecMxm NstkPoRecpts XML in")
        Console.WriteLine("")
        m_logger.WriteInformationLog(rtn & " :: Process of CytecMxm NstkPoRecpts Inbound")
        ...
        Dim sInputDir As String = "\\ims\SDIWebProcessorsXMLFiles\NstkPoRecpts"
        Try
            sInputDir = My.Settings("inputDirectory").ToString.Trim
        Catch ex As Exception
            sInputDir = "\\ims\SDIWebProcessorsXMLFiles\NstkPoRecpts"
        End Try
        ...
        Try
            If aSrcFiles.Length > 0 Then
                For I = 0 To aSrcFiles.Length - 1
                    If aSrcFiles(I).Name.Length > Len("CYTECMXM_NSTK_PO_RECPT_XML") - 1 Then
                        If aSrcFiles(I).Name.StartsWith("CYTECMXM_NSTK_PO_RECPT_XML") Then
                            File.Copy(aSrcFiles(I).FullName, "C:\CytecMxmIn\XMLIN\NstkPoRecpts\" & aSrcFiles(I).Name, True)
                            File.Delete(aSrcFiles(I).FullName)
                            m_logger.WriteInformationLog(rtn & " :: " & aSrcFiles(I).FullName & " moved to " & "C:\CytecMxmIn\XMLIN\NstkPoRecpts\" & aSrcFiles(I).Name)
            ...
            End If
        Catch ex As Exception
            myLoggr1.WriteErrorLog(rtn & " :: Error moving file(s) from " & dirInfo.FullName & " to C:\CytecMxmIn\XMLIN\NstkPoRecpts\ " & "...")
        ...
        Try
            bolError = GetCytecNstkPoRecptsIn()
        Catch ex As Exception
            myLoggr1.WriteErrorLog(rtn & " :: " & ex.ToString)
            bolError = True
        End Try
        ...
        m_logger.WriteInformationLog(rtn & " :: End of CytecMxm NstkPoRecpts IN Process")

    End Sub

    Private Function GetCytecNstkPoRecptsIn() As Boolean
        Dim bolError As Boolean = False
        Dim rtn As String = "CytecMxmNstkPoRecpts.GetCytecNstkPoRecptsIn"

        Console.WriteLine("Start Insert of CytecMxm NstkPoRecpts in ??? ")
        Console.WriteLine("")

        m_logger.WriteInformationLog(rtn & " :: Start Insert of CytecMxm NstkPoRecpts in ??? ")

        Dim dirInfo As DirectoryInfo = New DirectoryInfo("C:\CytecMxmIn\XMLIN\NstkPoRecpts\")
        Dim strFiles As String = ""
            ...
            If aFiles.Length > 0 Then
                For I = 0 To aFiles.Length - 1
                    ...
                    Catch exLoad As Exception
                        ...
                        bLineError = True
                        Try
                            File.Move(aFiles(I).FullName, "C:\INTFCXML\BadXML\NstkPoRecpts\" & aFiles(I).Name)
                            File.Delete(aFiles(I).FullName)
                        Catch ex24 As Exception
                            myLoggr1.WriteErrorLog(rtn & " :: ** ")
                            myLoggr1.WriteErrorLog(rtn & " :: Error (moving file to BadXML\NstkPoRecpts\ folder): " & ex24.Message & " in file " & aFiles(I).Name)
                            myLoggr1.WriteErrorLog(rtn & " :: ** ")
                        End Try
                    End Try
                    ...
                    If Trim(strXMLError) <> "" Or bolError Then
                        ...
                        File.Copy(aFiles(I).FullName, "C:\CytecMxmIn\BadXML\NstkPoRecpts\" & aFiles(I).Name, True)
                        File.Delete(aFiles(I).FullName)
                        m_logger.WriteInformationLog(rtn & " :: " & aFiles(I).FullName & " moved to " & "C:\CytecMxmIn\BadXML\NstkPoRecpts\" & aFiles(I).Name)
                    Else

                        'move file to XMLInProcessed folder
                        File.Copy(aFiles(I).FullName, "C:\CytecMxmIn\XMLINProcessed\NstkPoRecpts\" & aFiles(I).Name, True)
                        File.Delete(aFiles(I).FullName)
                        m_logger.WriteInformationLog(rtn & " :: " & aFiles(I).FullName & " moved to " & "C:\CytecMxmIn\XMLINProcessed\NstkPoRecpts\" & aFiles(I).Name)
    ...
    Private Sub SendEmail(Optional ByVal bIsSendOut As Boolean = False)
        ...
        Dim rtn As String = "CytecMxmNstkPoRecpts.SendEmail"
        ...
        email.Body &= "<table><tr><td>CytecMxmNstkPoRecpts has completed with "
        If bolWarning = True Then
            email.Body &= "warnings,"
            email.Subject = " (TEST) CytecMxmNstkPoRecpts Warning"
        Else
            email.Body &= "errors;"
            email.Subject = " (TEST) CytecMxmNstkPoRecpts Error"
        End If
                ...
                If Trim(m_arrXMLErrFiles) <> "" Then
                    ...
                    If arrErrFiles.Length > 0 Then
                        ...
                        For int1 = 0 To arrErrFiles.Length - 1
                            Dim myFileName2 As String = "C:\CytecMxmIn\BadXML\NstkPoRecpts\" & arrErrFiles(int1)
    ...
    Private Sub SendEmail1(ByVal mailer As System.Web.Mail.MailMessage)
        ...
        Try
            ...
            SendLogger(mailer.Subject, mailer.Body, "CYTECMXMNSTKPORCPTSIN", "Mail", mailer.To, mailer.Cc, mailer.Bcc)
