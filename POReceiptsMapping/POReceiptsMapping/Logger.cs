...
namespace POMapping
{
    class Logger
    {
        ...
        public void SendEmailAlert(string sLastErrorMessage)
        {
            try
            {
                const string cErrMsg = "Utility POReceiptMapping had a critical error";
                string strBodyhead = "";
                string strbodydetl = "";
                string strBody = "";
                POReceiptsMapping.SDiEmailUtilityService.EmailServices SDIEmailService = new POReceiptsMapping.SDiEmailUtilityService.EmailServices();
                ...
                try
                {
                    SDIEmailService.EmailUtilityServices("Mail", "sdiportalsupport@avasoft.biz","karthick.k.s@avasoft.com", "Error from POReceiptMapping Utility", "", "", strBody, "SDIERRMAIL", MailAttachmentName, MailAttachmentbytes.ToArray());
                }
                catch (Exception ex1)
                {
                }
