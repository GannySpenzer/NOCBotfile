...
namespace POReceiptsMapping
{
    class POReceiptsMappingDAL
    {
        ...
        /// </summary>
        /// <returns></returns>
        public DataTable getPOReceiptMappingData(Logger m_oLogger)
        {
            ...
            try
            {
                ...
                m_oLogger.LogMessage("getPOReceiptMappingData", "PeopleSoft connection string : " + OracleConString);
                m_oLogger.LogMessage("getPOReceiptMappingData", "Query To get the PO mapping date : " + strSQLstring);
                dtResponse = oleDBExecuteReader(strSQLstring);
                m_oLogger.LogMessage("getPOReceiptMappingData", "Number of rows Seleted " + dtResponse.Rows.Count);
            }
            catch (Exception ex)
            {
                m_oLogger.LogMessage("getPOReceiptMappingData", "Error trying to get the PO Receipt Mapping data.", ex);
        ...
        /// </summary>
        /// <returns></returns>
        public int UpdatePOReceiptMappingData(Logger m_oLogger)
        {
            ...
            try
            {
                ...
                m_oLogger.LogMessage("UpdatePOReceiptMappingData", "Number of rows updated : " + rowsAffected);

            }
            catch (Exception ex)
            {
                m_oLogger.LogMessage("UpdatePOReceiptMappingData", "Error trying to Update the PO Receipt Mapping data.", ex); throw;
            }
            return rowsAffected;
        }
